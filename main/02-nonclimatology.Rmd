...
Plot the distance matrix for EMD and RMSE (**Supplemental Figure 1, 2**).
...
**Supplemental Figure 5** of the paper, showing longer term trend in longer time range
...
```{r alternate-one-slice-explanation, fig.width = 15, fig.height = 5}
## Helper to remove coastline
remove_landlock <- . %>%
  dplyr::mutate(land = mark_land(lon, lat, overreact = TRUE)) %>%
  dplyr::filter(land == FALSE)  %>%
...
```{r alternative-one-slice, fig.width = 10, fig.height=3.5}
two_distmats = sapply(c("omd", "rmse"), function(dist_type){ 
...
What is causing this difference between the two distances? Compare January to (1) April or (2) August.
