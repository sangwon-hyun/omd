...
library(tidync)
source("00-helpers.R")
load_all("~/repos/omd/omd") ## Location of OMD R package; replace with library(omd) soon.
```
...
There are multiple resolutions of this data; we mainly make use of the 1-degree
resolution data.
...
TODO: add a simple plot here (pipe out from `rdat`).

# Non-climatology data

Non-climatology dataset was compiled by Bror Johnsson and shared as netcdf
(`.nc`) files. This data was further cleaned (code shown furtherbelow) and saved
...
is one of {1998, 1999, 2000, 2001}, and `mo` is month number 1, ..., 12.

```{r nonclimatology, fig.width = 5, fig.height = 5}
type = "real"
year = 2000
mo = 6
filename = paste0(type, "-nonclim-", year, "-", mo, ".RDS")
...
This data files (named e.g. `darwin-nonclim-2000-1.RDS`) were cleaned and
